...
int objloader::load(std::string filename,std::vector<collisionplane>* collplane)
{
...
void objloader::clean()
{
	...
	for(int i=0;i<materials.size();i++)
		delete materials[i];
	for(int i=0;i<texturevector3d.size();i++)
...
void objloader::smoothnormals()
{
	for(int i=1;i<vertex.size()+1;i++)
	{
		...
		vertexnormals.push_back(new vector3d(vecX,vecY,vecZ));
	}
}
